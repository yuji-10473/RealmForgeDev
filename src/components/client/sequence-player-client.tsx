...
export function SequencePlayerClient() {
  ...
  const [chars, setChars] = useState<Record<string, { x: number; y: number; data: VillagerData; targetIdx: number }>>({});
  ...
  const loadSequence = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('sequence.json の読み込みに失敗しました。');
  ...
  const initStoryStep = async (step: Step) => {
    try {
      ...
      // 2. Load Map (assuming worldId refers to mapId in this simplified version or a world structure)
      // For now, use mapId from story.json
      const mapRes = await fetch(`${sequencePath}/media/backgrounds/${story.mapId}.png`);
      if (mapRes.ok) setCurrentMapUrl(`${sequencePath}/media/backgrounds/${story.mapId}.png`);
      ...
      toast({ variant: 'destructive', title: 'ステップの初期化に失敗', description: 'データが見つかりません。' });
    }
  };
  ...
  useEffect(() => {
    ...
    const loop = () => {
      setChars(prev => {
        ...
        for (const id in next) {
          ...
          if (dist < moveSpeed) {
            ...
            if (target.eventId) {
              ...
              return next; // Pause loop while event is active
            }
          } else {
        ...
        if (allStopped) {
          // Move to next step if all characters finished their paths
  ...
  if (error) return <Card className="border-destructive"><CardHeader><CardTitle className="text-destructive flex items-center gap-2"><AlertCircle />エラー</CardTitle><CardDescription>{error}</CardDescription></CardHeader><CardContent><Button onClick={loadSequence}>再試行</Button></CardContent></Card>;
    ...
    return (
      <Card className="max-w-md mx-auto">
        <CardHeader>
          ...
          <CardDescription>エクスポートされたZIPを展開したパスを入力してください。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">※ 開発環境では public/ フォルダ内に配置されたデータを参照します。</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <Card className="max-w-2xl mx-auto">
          ...
          <CardContent className="flex flex-col items-center gap-6">
            <div className="text-center space-y-2">
              <p className="text-sm font-medium">構成: {sequence.steps.length} ステップ</p>
        ...
        <div className="relative flex-grow min-h-0 bg-black border-2 border-border overflow-hidden rounded-lg shadow-2xl">
          ...
          {step?.type === 'story' && (
            <div className="relative w-full h-full">
              ...
              {Object.entries(chars).map(([id, char]) => (
                <div
                  key={id}
                  className="absolute -translate-x-1/2 -translate-y-full transition-all duration-100"
                  style={{
                ...
                >
                  <Image src={`${sequencePath}/${char.data.imageUrl}`} alt={char.data.name} layout="fill" objectFit="contain" unoptimized />
          ...
          {step?.type === 'video' && (
            <div className="w-full h-full flex flex-col items-center justify-center bg-black">
              <h2 className="text-white text-xl mb-4">{step.videoTitle}</h2>
              <video
                src={`${sequencePath}/${step.videoUrl}`}
                className="max-w-full max-h-[80%]"
                autoPlay
                controls
                onEnded={handleNextStep}
              />
              <Button onClick={handleNextStep} variant="ghost" className="text-white mt-4">スキップ <ChevronRight /></Button>
            </div>
          )}
